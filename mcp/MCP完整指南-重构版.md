...
| 范围 | 存储位置 | 适用场景 | 官方定义 |
|------|---------|---------|----------|
| **local** | 当前目录`.claude/mcp.json` | 项目特定、敏感配置 | 项目特定用户设置中的默认配置级别 |
| **project** | 项目根目录`.mcp.json` | 团队共享、版本控制 | 存储在.mcp.json文件中，可提交到版本控制 |
| **user** | `~/.claude/mcp.json` | 个人工具、跨项目 | 跨项目的个人工具配置 |
...
| 配置方式 | 适用场景 | 优势 | 劣势 |
|---------|---------|------|------|
| **命令行配置** | 单个服务器、快速配置 | 简单直观、即时生效 | 批量操作不便 |
| **JSON文件配置** | 批量配置、团队协作 | 版本控制、批量管理 | 需要了解JSON格式 |
| **导入配置** | 迁移和批量导入 | 一键迁移、批量导入 | 依赖外部配置源 |
...
| 作用域 | 文件路径 | 使用场景 | 官方说明 |
|-------|---------|---------|----------|
| **local** | `.claude/mcp.json` (当前目录) | 项目特定配置，私有设置 | 存储在项目的用户特定设置中 |
| **project** | `.mcp.json` (项目根目录) | 团队共享，版本控制 | 可提交到版本控制的团队配置 |
| **user** | `~/.claude.json` (用户目录) | 个人全局配置 | 跨项目的个人工具配置 |

**注意**：用户范围的具体文件路径在官方文档中未明确说明，实际路径可能是`~/.claude.json`而非`~/.claude/mcp.json`
...
**适用场景**：从Claude Desktop迁移配置、从JSON文件批量导入、配置源迁移
