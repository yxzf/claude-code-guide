...
```python
# 服务器请求AI生成摘要
response = await client.sampling_complete(
    prompt="请总结这份数据报告",
    max_tokens=200
)
```
...
```python
# 服务器请求用户确认
confirmation = await client.elicitation_request(
    prompt="确认删除这个文件吗？",
    type="confirmation"
)
```
