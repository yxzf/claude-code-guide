...
**增强LLM (Augmented LLM)** = 基础LLM + 增强能力（检索、工具、记忆）

根据Anthropic的定义，增强LLM是所有agentic系统的基础构建块，它让LLM具备：
- **检索能力**：主动生成搜索查询获取信息
- **工具使用**：选择和调用合适的外部工具
- **记忆管理**：决定保留哪些重要信息

关键是LLM能**主动使用**这些能力，而不是被动调用。

基于这个共同基础，我们可以构建两种不同的agentic系统：

### 核心架构对比

| 维度 | Workflow | Agent |
|------|----------|--------|
| **编排方式** | 通过预定义代码路径编排LLM和工具 | LLM动态指导自己的流程和工具使用 |
| **控制权** | 代码逻辑控制执行流程 | LLM控制如何完成任务 |
| **适应性** | 固定路径，需重新编程改变 | 动态适应，自主调整策略 |
| **可预测性** | 高度可预测 | 相对不可预测 |
| **适用场景** | 明确定义的任务，追求一致性 | 开放性问题，需要灵活性 |
